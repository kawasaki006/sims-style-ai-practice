...
public class SimpleInteraction : BaseInteraction
{
    ...
    public override bool LockInteraction(CommonAIBase performer)
    {
        ...
        {
            Debug.LogError($"{performer.name} trying to local {_DisplayName} which is already at max user");
            return false;
        }
        ...
        // Only populate when an AI is performing this interaction, =null when selected but not performed
    ...
    protected virtual void Update()
    {
        ...
        foreach (var kvp in  CurrentPerformers)
        {
            ...
            {
                continueInteraction = ApplyInteractionEffects(performer, (performerInfo.ElapsedTime - previousElaspedTime) / _Duration, isFinalTick);
            }
